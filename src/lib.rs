...
pub fn extract_info_from_xml(file_path: &PathBuf) -> Result<CteInfo, MyError> {
    ...
    let mut complementary_ctes: HashMap<String, HashSet<String>> = HashMap::new();

    for cap in REGEX_CHAVE_CTE.captures_iter(&all_data) {
    ...
    for cap in REGEX_CHAVE_CTE_COMPL.captures_iter(&all_data) {
        ...
        let chave_cte_complementar: &str = cap.get(2).map_or("", |m| m.as_str());
        let fechamento: &str = cap.get(3).map_or("", |m| m.as_str());

        if (origin != fechamento) || origin.is_empty() || chave_cte_complementar.is_empty() {
            continue;
        }

        if verify_key_code(file_path, chave_cte_complementar, CODE_CTE)? {
            complementary_ctes
                .entry(chave_cte_complementar.to_string())
                .or_default()
    ...
    */
    ...
    Ok((chaves_cte, complementary_ctes)) // (key, value)
}
    ...
    array_of_files
        ...
        .fold(BTreeMap::new, |mut acc: MapCteComp, (key, value)| {
            for chave_cte in key {
                for (complementary_cte, origins) in &value {
                    acc.entry(chave_cte.clone())
                        .or_default()
                        .entry(complementary_cte.clone())
                        .or_default()
        ...
        .reduce(BTreeMap::new, |mut acc, map| {
            map.into_iter().for_each(|(key, values)| {
                for (complementary_cte, origins) in values {
                    acc.entry(key.clone())
                        .or_default()
                        .entry(complementary_cte)
                        .or_default()
                        .extend(origins);
                }
            });
...
pub fn print_output(
    ...
    let mut count_com: HashSet<&String> = HashSet::new();

    for (cte, btree_map) in complementary_key_for_cte {
        btree_map
            .iter()
            .filter(|(cte_complementar, _origins)| *cte_complementar != cte)
            .try_for_each(|(cte_complementar, origins)| -> Result<(), MyError> {
                let size = origins.len();
                let origins: Vec<&String> = origins.iter().collect();

                writeln!(
                    output_file,
                    " $chave_complementar_deste_cte{{ '{cte}' }}{{ '{cte_complementar}' }} = {size} ; origem_da_informacao = {origins:?}"
                )
                .map_err(|e| MyError::FileWriteError(config.output_filename.clone(), e))?;

                count_com.insert(cte_complementar);
    ...
    println!(
        "Encontrado {} CTes contendo no total {} CTes Complementares.",
        count_cte.len(),
        count_com.len()
    );
    println!("Todas estas chaves complementares de conhecimento de transporte foram vinculadas aos CTes correspondentes.");
...
mod lib_functions {
    ...
    fn read_xml_content() -> Result<(), Error> {
        ...
        let btree_set = BTreeSet::from(["chCTeAnulacao".to_string(), "refCteAnu".to_string()]);

        let mut btree_map: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
        btree_map.insert(
            "35210123456789012345570000000112233445566333".to_string(),
            btree_set,
        );
